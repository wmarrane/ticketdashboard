...
| 192.168.56.132 | Docker + Docker Compose; portas 80 e 3001 livres |
...
- **backend** — build de `backend/Dockerfile`, porta 3001, configurado pelo `.env`.
...
| `docker compose up` falha com "port is already allocated" | Porta 80 ou 3001 ocupada no 132 | Identificar o processo (`docker ps`, `ss -ltnp`) e liberar a porta, ou ajustar o mapeamento em `docker-compose.yml` |
